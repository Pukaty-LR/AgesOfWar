...
export class UI {
  ...
  renderScoreboard(game) {
    ...
    let king = -1; { const big = [...game.ents.values()].find(e => e.k === 'm' && e.big); if (big) { const near = {}; for (const e of game.ents.values()) if (e.k === 'u' && !e.hd && e.o !== undefined && !game.players[e.o].neutral && Math.hypot(e.x - big.x, e.y - big.y) < 10) near[e.o] = (near[e.o] || 0) + 1; let best = 0; for (const [o, n] of Object.entries(near)) if (n > best) { best = n; king = +o; } } }
    ...
    tbl.innerHTML = t('<tr><th>Hráč</th><th>Frakce</th><th>Tým</th><th>Pop</th><th>Zabito</th><th>Zničeno</th><th>Skóre</th></tr>').replace('</tr>', allies ? `<th>${t('Tribut')}</th></tr>` : '</tr>') + rows.map(({ p, sc }) => `<tr style="${p.alive ? '' : 'opacity:.5'}"><td style="color:${TEAM_COLORS[p.color].hex}">${p.name}${p.isAI ? ' (AI)' : ''}${p.alive ? '' : t(' (vyřazen)')}${p.id === king ? ` <span class="winner">${t('DRŽÍ STŘED')}</span>` : ''}</td><td>${fname(p.faction)}</td><td>${p.team + 1}</td><td>${p.pop}/${p.popCap}</td><td>${p.stats.unitsKilled}</td><td>${p.stats.buildingsRazed}</td><td><b>${sc}</b></td>${tribute(p)}</tr>`).join('');
