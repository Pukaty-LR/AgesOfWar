...
export class UI {
  ...
  renderScoreboard(game) {
    ...
    tbl.innerHTML = t('<tr><th>Hráč</th><th>Frakce</th><th>Tým</th><th>Pop</th><th>Zabito</th><th>Zničeno</th><th>Skóre</th></tr>') + rows.map(({ p, sc }) => `<tr style="${p.alive ? '' : 'opacity:.5'}"><td style="color:${TEAM_COLORS[p.color].hex}">${p.name}${p.isAI ? ' (AI)' : ''}${p.alive ? '' : t(' (vyřazen)')}${p.id === king ? ` <span class="winner">${t('DRŽÍ STŘED')}</span>` : ''}</td><td>${fname(p.faction)}</td><td>${p.team + 1}</td><td>${p.pop}/${p.popCap}</td><td>${p.stats.unitsKilled}</td><td>${p.stats.buildingsRazed}</td><td><b>${sc}</b></td></tr>`).join('');
  }
