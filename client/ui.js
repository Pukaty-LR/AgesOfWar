...
export class UI {
  ...
  renderScoreboard(game) {
    ...
    // who holds the endless centre: the player with most units within 10 tiles of it
    let king = -1, kingSpots = 0; { const bigs = [...game.ents.values()].filter(e => e.k === 'm' && e.big); const held = {}; for (const big of bigs) { const near = {}; for (const e of game.ents.values()) if (e.k === 'u' && !e.hd && e.o !== undefined && !game.players[e.o].neutral && Math.hypot(e.x - big.x, e.y - big.y) < 10) near[e.o] = (near[e.o] || 0) + 1; let best = 0, who = -1; for (const [o, n] of Object.entries(near)) if (n > best) { best = n; who = +o; } if (who >= 0) held[who] = (held[who] || 0) + 1; } for (const [o, n] of Object.entries(held)) if (n > kingSpots) { kingSpots = n; king = +o; } }
    const me = game.players[game.me]; const rn = ERAS[game.era].resources; const allies = me && !game.gameOver ? rows.filter(({ p }) => p.id !== me.id && p.team === me.team && p.alive && !p.neutral).length : 0;
    const tribute = p => allies && p.id !== me.id && p.team === me.team && p.alive && !p.neutral ? `<td class="tb-cell"><button class="tb" data-to="${p.id}" data-r="p" title="${t('Poslat spojenci suroviny (Shift = 500)')}">+100 ${rn.p.name}</button> <button class="tb" data-to="${p.id}" data-r="s" title="${t('Poslat spojenci suroviny (Shift = 500)')}">+100 ${rn.s.name}</button></td>` : (allies ? '<td></td>' : '');
    tbl.innerHTML = t('<tr><th>Hráč</th><th>Frakce</th><th>Tým</th><th>Pop</th><th>Zabito</th><th>Zničeno</th><th>Skóre</th></tr>').replace('</tr>', allies ? `<th>${t('Tribut')}</th></tr>` : '</tr>') + rows.map(({ p, sc }) => `<tr style="${p.alive ? '' : 'opacity:.5'}"><td style="color:${TEAM_COLORS[p.color].hex}">${p.name}${p.isAI ? ' (AI)' : ''}${p.alive ? '' : t(' (vyřazen)')}${p.id === king ? ` <span class="winner">${t('DRŽÍ STŘED')}${kingSpots > 1 ? ' ×' + kingSpots : ''}</span>` : ''}</td><td>${fname(p.faction)}</td><td>${p.team + 1}</td><td>${p.pop}/${p.popCap}</td><td>${p.stats.unitsKilled}</td><td>${p.stats.buildingsRazed}</td><td><b>${sc}</b></td>${tribute(p)}</tr>`).join('');
    tbl.querySelectorAll('button.tb').forEach(b => { b.onmousedown = ev => ev.stopPropagation(); b.onclick = ev => { ev.stopPropagation(); game.tribute(+b.dataset.to, b.dataset.r, ev.shiftKey ? 500 : 100); this.lastSbAt = performance.now(); }; });
  }
  ...
  hoverChanged(game, e) { if (!e) { this.hideTooltip(); return; } if (game.state.placing) return; const name = game.entName(e); let html = `<b>${name}</b>`; if (e.o !== undefined) html += `<div>${game.players[e.o].name}</div>`; if (e.k === 't' || e.k === 'm') html += `<div class="desc">${t('Zbývá')}: ${e.big ? '∞' : e.a}</div>`; if ((e.k === 'u' || e.k === 'b') && (e.lv || 1) > 1) html += `<div class="desc">${t('Úroveň')} ${e.lv}</div>`; if (e.hp !== undefined && e.m) html += `<div class="desc">${e.hp} / ${e.m} HP</div>`; this.showTooltip(html, game.state.mouse.x + 16, game.state.mouse.y + 16); }
