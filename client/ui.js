...
export class UI {
  ...
  updateSelection(game) {
    ...
    if (sel.length === 1) {
      ...
      if (e.k === 'u') { const d = game.unitDef(e); const pr = game.players[e.o].research || {}; let dmg = d.dmg, arm = d.armor; for (const [rid, rd] of Object.entries(RESEARCH)) { const l = pr[rid] || 0; if (!l || !rd.roles.includes(d.role)) continue; if (rd.dmgAdd) dmg += rd.dmgAdd * l; if (rd.dmgMul) dmg *= Math.pow(rd.dmgMul, l); if (rd.armorAdd) arm += rd.armorAdd * l; } if (d.aura) dmg *= 1 + 0.1 * ((e.lv || 1) - 1); stat('attack', 'Útok', withBonus(d.dmg, dmg)); stat('armor', 'Pancíř', withBonus(d.armor, arm)); stat('range', 'Dosah', d.range >= 1 ? d.range.toFixed(1) : 'blízko'); stat('speed', 'Rychlost', d.speed.toFixed(1)); if (e.c) stat('carry', 'Nese', e.c === 'p' ? game.eraDef.resources.p.name : game.eraDef.resources.s.name); }
      else if (e.k === 'b') { const d = game.buildingDef(e); let bArm = d.armor, bDmg = d.attack ? d.attack.dmg : 0, bRange = d.attack ? d.attack.range : 0; for (const up of d.upgrades || []) if (up.level <= (e.lv || 1)) { if (up.armorAdd) bArm += up.armorAdd; if (up.dmgMul) bDmg *= up.dmgMul; if (up.rangeAdd) bRange += up.rangeAdd; } stat('armor', 'Pancíř', withBonus(d.armor, bArm)); if (d.attack) { stat('attack', 'Útok', withBonus(d.attack.dmg, bDmg)); stat('range', 'Dosah', withBonus(d.attack.range, bRange)); } if (d.popCap) stat('pop', 'Populace', '+' + d.popCap); if (!e.bl) stat('build', 'Stavba', Math.round(e.pr * 100) + ' %'); }
  ...
  buildCommandCard(game, sel) {
    ...
    if (units.length) {
      ...
      if (this.buildMenu && hasWorker) {
        ...
        btn(base + 0, { label: 'Přesun', key: 'M', icon: () => actionIcon('move'), desc: 'Přesunout na místo (nebo pravé tlačítko).', act: () => { game.state.mode = 'move'; } });
        btn(base + 1, { label: 'Stop', key: 'S', icon: () => actionIcon('stop'), desc: 'Zastavit současný rozkaz.', act: () => game.orderStop() });
        btn(base + 2, { label: 'Držet', key: 'H', icon: () => actionIcon('hold'), desc: 'Držet pozici, nepronásledovat.', act: () => game.orderHold() });
        if (hasMil || hasWorker) btn(base + 3, { label: 'Útok', key: 'A', icon: () => actionIcon('amove'), desc: 'Útočný pochod: útočí na vše cestou. Klik na nepřítele = útok na cíl.', act: () => { game.state.mode = 'amove'; }, active: () => game.state.mode === 'amove' });
        if (hasWorker) {
          const rp = game.eraDef.resources.p, rs = game.eraDef.resources.s;
          const wb = hasMil ? 4 : 0;
          btn(wb + 1, { label: 'Těžit ' + (rp.acc || rp.name.toLowerCase()), key: 'G', icon: () => actionIcon('gather', 64, rp.color), desc: `Vybraní dělníci jdou těžit ${rp.acc || rp.name.toLowerCase()} z nejbližšího naleziště (${game.eraDef.nodes.mine.name.toLowerCase()}).`, act: () => game.gatherKind('mine') });
          btn(wb + 2, { label: 'Těžit ' + (rs.acc || rs.name.toLowerCase()), key: 'F', icon: () => actionIcon('gather', 64, rs.color), desc: `Vybraní dělníci jdou těžit ${rs.acc || rs.name.toLowerCase()} z nejbližšího lesa.`, act: () => game.gatherKind('tree') });
          btn(wb + 0, { label: 'Stavět', key: 'B', icon: () => actionIcon('build'), desc: 'Otevře nabídku staveb.', act: () => { this.buildMenu = true; this.selectionChanged = true; this.dirty = true; this.lastSig = ''; } });
          btn(wb + 3, { label: 'Opravit', key: 'R', icon: () => actionIcon('repair'), desc: 'Klikni na poškozenou vlastní budovu (nebo rovnou pravým tlačítkem).', act: () => { game.state.mode = 'repair'; }, active: () => game.state.mode === 'repair' });
        }
      }
    } else if (blds.length) {
      ...
      if (!b.bl) { btn(11, { label: 'Zrušit stavbu', key: 'X', icon: () => actionIcon('cancel'), cls: 'cancel', desc: 'Zruší stavbu, vrátí 75 % surovin.', act: () => game.cancelBuild(b.i) }); }
        ...
        { const bdef = game.tech.buildings[b.t]; let slot = trains.length; for (const up of bdef.upgrades || []) { if (up.level <= (b.lv || 1)) continue; for (const lu of up.unlocks) { if (slot >= 6) break; const ud = game.tech.units[lu]; if (!ud) continue; btn(slot++, { label: ud.name, icon: () => unitPortrait(ud.sprite, p.color, 64, { faction: p.faction }), cost: ud.cost, cls: 'locked', desc: `Zamčeno – vyžaduje ${bdef.name} úrovně ${up.level}${up.hall ? ` (a radnici úrovně ${up.hall})` : ''}.`, act: () => { game.ui.alert(`Vyžaduje ${bdef.name} úrovně ${up.level}.`, true); }, canAfford: () => false }); } } }
        trains.forEach((u, i) => { const ud = game.tech.units[u]; const bon = Object.entries(ud.bonus || {}).filter(([, v]) => v > 1).map(([k, v]) => `${ROLE[k] || k} ×${v}`).join(', '); btn(i, { label: ud.name, key: keys[i], icon: () => unitPortrait(ud.sprite, p.color, 64, { faction: p.faction }), cost: ud.cost, desc: `${ud.desc ? ud.desc + ' ' : ''}${ud.heal ? `Léčí ${ud.heal.amount} HP/s v okruhu ${ud.heal.range}. ` : `Útok ${ud.dmg}, `}pancíř ${ud.armor}, HP ${ud.hp}, ${ud.heal ? '' : `dosah ${ud.range >= 1 ? ud.range.toFixed(1) : 'na blízko'}, `}rychlost ${ud.speed.toFixed(1)}.${bon ? ' Bonus proti: ' + bon + '.' : ''} Populace ${ud.pop}. Výcvik ${Math.round(ud.trainTime)} s.${ud.unique ? ' Jen jeden.' : ' Shift = 5×.'}`, repeat: !ud.unique, act: () => game.train(u), canAfford: () => game.players[game.me].res.p >= ud.cost.p && game.players[game.me].res.s >= ud.cost.s }); });
        const up = game.nextUpgrade(b);
        if (up) { const need = up.hall && game.hallLevel() < up.hall; const unlockNames = up.unlocks.map(u => game.tech.units[u]?.name).filter(Boolean); btn(8, { label: `Vylepšit (${up.level})`, key: 'U', icon: () => actionIcon('upgrade'), cost: up.cost, desc: `Vylepší budovu na úroveň ${up.level} (${Math.round(up.time)} s).${up.desc ? ' ' + up.desc : ''}${unlockNames.length ? ' Odemkne: ' + unlockNames.join(', ') + '.' : ''}${up.popCap ? ` +${up.popCap} populace.` : ''}${need ? ` Vyžaduje radnici úrovně ${up.hall}.` : ''}`, act: () => game.upgrade(b.i), canAfford: () => !need && game.players[game.me].res.p >= up.cost.p && game.players[game.me].res.s >= up.cost.s && !(b.q || []).some(q => q.t === '__up') }); }
