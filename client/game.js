...
export class Game {
  ...
  onEvent(ev) {
    ...
    switch (ev.t) {
      ...
      case 'upgraded': if (mine && ev.ty === 'hall') { this.audio.sfx('horn', 0.9); this.ui.banner(this.eraDef.hallNames[ev.level - 1] || '', tf('Radnice vylepšena na úroveň %1', ev.level)); } // Age of Empires style age-up fanfare
      if (mine) { this.audio.sfx('buildingDone', 0.8); const b = this.ents.get(ev.id); if (b) b.lv = ev.level; this.ui.alert(tf(t('%1: vylepšeno na úroveň %2'), this.tech.buildings[ev.ty]?.name, ev.level), false); this.ui.lastSig = ''; this.ui.dirty = true; } break;
