...
class App {
  ...
  renderCodex() {
    ...
    const ROLE = { infantry: t('pěchota'), ranged: t('střelci'), cavalry: 'jezdectvo/vozidla', siege: t('obléhací'), ship: t('lodě'), building: t('budovy'), wall: t('hradby'), worker: t('dělníci'), support: t('podpora') };
    ...
    if (this.codexTab === 'units') {
      for (const [k, u] of Object.entries(tech.units)) {
        ...
        const unlock = u.creep ? t('neutrální hlídač dolů (nelze cvičit)') : k === 'worker' || ['infantry', 'ranged', 'cavalry', 'siege', 'ship', 'transport'].includes(k) ? `${tech.buildings[u.building]?.name}` : (() => { for (const [bk, b] of Object.entries(tech.buildings)) for (const up of b.upgrades || []) if (up.unlocks.includes(k)) return tf('%1 úroveň %2', b.name, up.level); return '?'; })();
        card(unitPortrait(u.sprite, 0, 88, { faction: e.factions[0].id }), u.name, `${ROLE[u.role]} · ${unlock}${u.unique ? ' · hrdina' : ''}`, [[t('HP'), u.hp], ...(u.heal ? [[t('Léčení'), u.heal.amount + ' HP/s']] : [[t('Útok'), u.dmg]]), [t('Pancíř'), u.armor], ...(u.heal || u.capacity ? [] : [[t('Dosah'), u.range >= 1 ? u.range.toFixed(1) : t('blízko')]]), ...(u.capacity ? [[t('Kapacita'), u.capacity + ' jednotek']] : []), [t('Rychlost'), u.speed.toFixed(1)], [t('Cena'), `${u.cost.p} ${e.resources.p.short} / ${u.cost.s} ${e.resources.s.short}`], [t('Populace'), u.pop], [t('Výcvik'), Math.round(u.trainTime) + ' s']], (u.desc || '') + (u.heal ? tf(' Léčí %1 HP/s v okruhu %2.', u.heal.amount, u.heal.range) : '') + (bon ? tf(' Bonus proti: %1.', bon) : '') + (u.aura ? tf(' Aura +%1 % útok v okruhu %2.', Math.round((u.aura.dmg - 1) * 100), u.aura.range) : '') + (u.ability ? ` Schopnost: ${u.ability.name} – ${u.ability.desc}` : ''));
      }
    } else if (this.codexTab === 'buildings') {
      for (const [k, b] of Object.entries(tech.buildings)) {
        if (k === 'gate') continue;
        const ups = (b.upgrades || []).map(up => `${t('úroveň')} ${up.level}: ${up.cost.p}/${up.cost.s}${up.unlocks.length ? ' → ' + up.unlocks.map(u => tech.units[u]?.name).join(', ') : ''}${up.desc ? ' – ' + up.desc : ''}${up.popCap ? ` +${up.popCap} pop` : ''}`).join('; ');
        card(buildingPortrait(b.sprite, b.w, b.h, 0, 88, era), b.name, `${b.w}×${b.h}${b.trains.length ? t(' · cvičí: ') + b.trains.map(u => tech.units[u]?.name).join(', ') : ''}`, [[t('HP'), b.hp], [t('Pancíř'), b.armor], [t('Cena'), `${b.cost.p} ${e.resources.p.short} / ${b.cost.s} ${e.resources.s.short}`], [t('Stavba'), b.buildTime + ' s'], ...(b.attack ? [[t('Útok'), b.attack.dmg], [t('Dosah'), b.attack.range]] : []), ...(b.popCap ? [[t('Populace'), '+' + b.popCap]] : [])], (b.desc || '') + (ups ? tf(' Vylepšení – %1.', ups) : ''));
      }
    } else {
  ...
  drawMapPreview(style, sizeKey, era) {
    ...
    for (let i = 0; i < size * size; i++) { const t = map.tiles[i]; let col; switch (t) { case 0: col = pal.grass; break; case 5: col = pal.rock; break; case 1: col = pal.dirt; break; case 2: col = pal.sand; break; case 3: col = pal.water.map((v, k) => v * 0.7 + pal.sand[k] * 0.3); break; case 4: col = pal.deep; break; default: col = pal.rock; } const k = 0.8 + (map.height[i] - 0.5) * 0.5; img.data[i * 4] = col[0] * k; img.data[i * 4 + 1] = col[1] * k; img.data[i * 4 + 2] = col[2] * k; img.data[i * 4 + 3] = 255; }
  ...
  renderSaves(list) {
    ...
    for (const s of list) {
      ...
      tr.innerHTML = `<td>${s.name === 'autosave' ? t('<i>Automatické uložení</i>') : esc(s.name)}</td><td>${ERAS[s.era]?.name || '?'}</td><td>${Math.floor(t / 60)}:${String(t % 60).padStart(2, '0')}</td><td>${when.toLocaleDateString(langDef().locale)} ${when.toLocaleTimeString(langDef().locale, { hour: '2-digit', minute: '2-digit' })}</td><td></td>`;
