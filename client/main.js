  ...
  renderCodex() {
    ...
    if (this.codexTab === 'units') {
      for (const [k, u] of Object.entries(tech.units)) {
        ...
        card(unitPortrait(u.sprite, 0, 88, { faction: e.factions[0].id }), u.name, `${ROLE[u.role]} · ${unlock}${u.unique ? ' · hrdina' : ''}`, [['HP', u.hp], ...(u.heal ? [[t('Léčení'), u.heal.amount + ' HP/s']] : [[t('Útok'), u.dmg]]), [t('Pancíř'), u.armor], ...(u.heal || u.capacity ? [] : [[t('Dosah'), u.range >= 1 ? u.range.toFixed(1) : t('blízko')]]), ...(u.capacity ? [[t('Kapacita'), u.capacity + ' jednotek']] : []), [t('Rychlost'), u.speed.toFixed(1)], ['Cena', `${u.cost.p} ${e.resources.p.short} / ${u.cost.s} ${e.resources.s.short}`], [t('Populace'), u.pop], [t('Výcvik'), Math.round(u.trainTime) + ' s']], (u.desc || '') + (u.heal ? ` Léčí ${u.heal.amount} HP/s v okruhu ${u.heal.range}.` : '') + (bon ? ` Bonus proti: ${bon}.` : '') + (u.aura ? ` Aura +${Math.round((u.aura.dmg - 1) * 100)} % útok v okruhu ${u.aura.range}.` : '') + (u.ability ? ` Schopnost: ${u.ability.name} – ${u.ability.desc}` : ''));
      }
