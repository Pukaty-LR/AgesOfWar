...
- **Stálý odkaz pro kamarády: https://pukaty-lr.github.io/AgesOfWar/** – stránka na GitHub Pages, která vždy přesměruje na aktuálně běžící server. Server spuštěný s `--public` si svou adresu sám zapíše do `docs/current.json` a odešle do repozitáře, takže odkaz zůstává stejný, i když se adresa tunelu mění.
- **Hraní přes internet z vlastního PC**: `start-public.bat` (nebo `node server/index.js --public`) spustí server a sám otevře Cloudflare Quick Tunnel (zdarma, bez účtu, bez hesla). Aktuální veřejná adresa `https://xxxx.trycloudflare.com` je vidět přímo v menu hry („Adresa pro kamarády“ + Kopírovat) a v okně serveru. Adresa se mění při každém spuštění – po restartu počítače pošli kamarádům novou. Při prvním spuštění se stáhne oficiální `cloudflared` do `bin/`.
- **Hosting 24/7 zdarma (běží i bez tvého PC)**: server je obyčejná Node aplikace (`npm start`, port z proměnné `PORT`), přiložený `render.yaml` je hotový blueprint pro render.com. Postup: 1) na github.com založ prázdný repozitář (např. `AgesOfWar`) a nahraj kód: `git remote add origin https://github.com/TVOJE_JMENO/AgesOfWar.git` a `git push -u origin master`; 2) na render.com se přihlas přes GitHub, zvol **New → Blueprint**, vyber repozitář a potvrď – za pár minut běží hra na stálé adrese `https://ages-of-war.onrender.com` (název lze změnit). Bezplatná služba po 15 minutách bez hráčů usne a první návštěvník čeká ~30 s na probuzení; uložené hry proti AI na serveru nepřežijí restart služby.
...
- **Radnice sama vynáší** +10 zlata (ropy / krystalů) a +5 dřeva každých 15 s, s úrovní radnice víc – pojistka, aby hra neskončila, když dojdou dělníci i doly.
