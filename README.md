...
- **Hraní přes internet**: `start-public.bat` spustí server a otevře ho do světa přes localtunnel (bez účtů). Tunel vypíše veřejnou adresu typu `https://xxxx.loca.lt`, tu pošli kamarádům; při první návštěvě zadají „tunnel password“ = tvoje veřejná IP (skript ji vypíše, jinak `curl https://loca.lt/mytunnelpassword`). Klient se připojuje WebSocketem na stejnou adresu (wss), lobby, chat i hra fungují beze změny. Adresa se při každém spuštění mění; `npx localtunnel --port 8080 --subdomain mojehra` si vyžádá pevný název. Trvalé nasazení: server je obyčejná Node aplikace (`npm start`, port z proměnné `PORT`), přiložený `render.yaml` je blueprint pro bezplatnou službu na render.com (repozitář na GitHub → New → Blueprint).
