...
export class AIPlayer {
  ...
  update() {
    ...
    if (tick % 100 === 0 && workers.length >= 6) {
      ...
      const tree = sim.nearestNode({ x: hall.x, y: hall.y, owner: this.pid }, 'tree', 22);
      if (p.res.s < 200 && p.res.p > 400 && onGold.length > 3 && tree) sim.command(this.pid, { t: 'gather', ids: [onGold[0].id], targetId: tree.id });
      else if (p.res.p < 200 && p.res.s > 400 && onWood.length > 3 && mineNode) sim.command(this.pid, { t: 'gather', ids: [onWood[0].id], targetId: mineNode.id });
    }
    // train workers
